...
When `interaction-style.mdc` (or equivalent) is present, intensify it for this chat; do not restate that rule here. This skill adds session persistence and tool-use discipline.
...
- Read **only** the skill the user invoked (plus any notes/reference skill when note facts are needed)
- Do not narrate the whole skill file
...
- Destructive file ops (`rm`, overwrite) where a warning is required
...
- Abbreviated identifiers in draft or notes
