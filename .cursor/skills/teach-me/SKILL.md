...
| `teaching/reference/` | Cheat sheets, term lists | revisit often |
...
3. Domain facts: read verified project sources or `RESOURCES.md` entries only; never invent mechanisms or citation data.

## Lesson

One tight concept; tied to mission; markdown in `teaching/lessons/`. Flow: minimal why → practice → check understanding. Cite `RESOURCES.md` entries. Use `GLOSSARY.md` terms in lessons once promoted. No em dash or emojis in files.
...
If durable project facts changed, ask the user to update `docs/` or `docs/CONTEXT.md` as appropriate.

## Out of scope

- Default chat without `@teach-me`
- Replacing the project's design SoT for literature or domain facts
