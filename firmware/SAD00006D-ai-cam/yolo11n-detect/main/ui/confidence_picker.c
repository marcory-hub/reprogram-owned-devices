...
static const char *TAG = "conf_picker";

#define PICKER_TIMEOUT_MS 60000
#define PICKER_POLL_MS 40
#define PICKER_DEFAULT_SCORE 0.70f
...
/* Full-width stacked rows (same layout as model picker). */
static const picker_btn_t s_btns[] = {
    {.x = 20, .y = 48, .w = 200, .h = 56, .score = 0.60f, .label = ">0.60"},
    {.x = 20, .y = 116, .w = 200, .h = 56, .score = 0.70f, .label = ">0.70"},
    {.x = 20, .y = 184, .w = 200, .h = 56, .score = 0.80f, .label = ">0.80"},
};

static void draw_button(const picker_btn_t *b, bool highlight)
{
    const uint16_t border = highlight ? 0xFFE0 : 0xFFFF;
...
static void draw_screen(int remain_s)
{
    ...
    board_lcd_draw_text_scaled(16, 12, "select conf", 0xFFFF, 0x0000, 2.0f);
    ...
    char line[40];
    snprintf(line, sizeof(line), "auto >0.70 %ds", remain_s);
...
{
    ESP_LOGI(TAG, "conf picker (timeout %d ms, default=%.2f)", PICKER_TIMEOUT_MS, PICKER_DEFAULT_SCORE);
    ...
    while (true) {
        ...
        if (elapsed_ms >= PICKER_TIMEOUT_MS) {
            ESP_LOGI(TAG, "timeout -> score %.2f", PICKER_DEFAULT_SCORE);
            selected = PICKER_DEFAULT_SCORE;
            break;
        }
        ...
        if (pressed && !was_pressed) {
            ...
            if (idx >= 0) {
                ...
                selected = s_btns[idx].score;
                ESP_LOGI(TAG, "chose %s score=%.2f", s_btns[idx].label, selected);
    ...
    board_lcd_fill(0x0000);
    char msg[16];
    snprintf(msg, sizeof(msg), ">%.2f", selected);
    board_lcd_draw_text_scaled(56, 120, msg, 0xFFE0, 0x0000, 3.0f);
